<%@ include file="header.jsp"%>

<div
	style="width: 900px; margin: 0 auto; background: #fff; box-shadow: 0 1px 5px #000;">
	<div style="padding: 10px 30px 10px 30px;">
		<%@ include file="navigation.jsp"%>
		<br> <br>
		<div style="">

			<h4>
				Logged in as:
				<sec:authentication property="name" />
				<sec:authentication property="authorities" />
			</h4>
			<div style="background: #555; color: white; padding: 10px;">
				<b>Update Customer Transaction</b>
			</div>
			<br>
			<form:form method="post" modelAttribute="externalTransaction"
				action="/SecureOnlineBanking/transactionMonitoringEmployee/updateExternalUserTransaction.html">
				<c:if test="${not empty errors}">
					<tr>
						<td colspan="3"><p class="dberrors">${errors}</p></td>
					</tr>
				</c:if>

				<table>


					<tr>
						<td><form:label path="transType">Transaction Type</form:label></td>
						<td><form:input path="transType" readonly="true" /></td>
					</tr>
					<tr>
						<td><form:label path="transDetail">Description </form:label></td>
						<td><form:input path="transDetail" /></td>
					</tr>
					<tr>
						<td><form:label path="amountInvolved">Amount </form:label></td>
						<td><form:input path="amountInvolved" readonly="true" /></td>
					</tr>
					<tr>
						<td><form:label path="status">Status</form:label></td>
						<td><form:select path="status">
								<form:option value="Pending" label="Pending" />
								<form:option value="Approved" label="Approved" />
							</form:select></td>
					</tr>
					<tr>
						<form:hidden path="transId" />
					</tr>
					<tr>
						<td></td>
						<td><input type="submit" value="Update Transaction"
							class="myButton"></td>
					</tr>
				</table>


			</form:form>
		</div>
		<br> <br>

	</div>
</div>
<%@ include file="footer.jsp"%>